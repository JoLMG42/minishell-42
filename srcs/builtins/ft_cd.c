...
/*   Updated: 2024/07/11 15:37:04 by juliensarda      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void update_pwd(t_shell *shell, t_data *data, char cwd[])
{
    ...
    if (!new_tab[0])
        return(ft_errors_exec(1, strerror(errno), shell, "malloc", errno), freetab(new_tab));
...
void	ft_cd(t_data *data, t_shell *shell, char **args)
{
	...
	if (count_args(data->args) >= 3)
	 	return (ft_errors_exec(1, " too many arguments", shell, "cd", 1));
	if (!data->args[1])
	{
		if (check_key(shell->envp, "HOME"))
			return (ft_errors_exec(1, " HOME not set", shell, "cd", 1));
		cd_ret = chdir(get_path_value(shell, "HOME"));
		if (cd_ret != 0)
			return(ft_errors_exec(1, strerror(errno), shell, "chdir", errno));
